...
private:
  ...
  static constexpr unsigned long EAGAIN_WRITE_BACKOFF_MS =
      5; // After SSH write EAGAIN AND toRemote at high-water, defer this
         // channel's next write attempt by this many ms so sibling
         // channels can claim the freed session capacity. Targets the
         // G2 starvation pattern where one slow peer's outbound ring
         // stays full and monopolises the SSH session's TCP send buffer.
