...
bool SSHTunnel::initializeSSH() {
    ...
    // Handshake
    int rc;
    while ((rc = libssh2_session_handshake(session, sshClient.fd())) == LIBSSH2_ERROR_EAGAIN) {
        delay(10);
    }
...
bool SSHTunnel::handleNewConnection() {
    ...
    channels[channelIndex].localClient = localClient;
...
void SSHTunnel::handleChannelData(int channelIndex) {
    ...
    if (!ch.active || !ch.channel) return;
    ...
    if (rc > 0) {
        size_t written = ch.localClient.write(rxBuffer, rc);
        if (written > 0) {
